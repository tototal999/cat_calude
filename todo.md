...
- [x] 103/103 業務測試、8/8 發布驗證及 EXE／manifest SHA-256 對照通過。
...
- [x] P6-9. PPTX 新增「快速摘要（抽樣）」與「完整分析（全部投影片）」；完整模式每 6 張分批，
      全部成功才回報完整 coverage，批次失敗保留部分 coverage，無文字投影片仍計數並明示
      圖片／圖表未分析。（2026-07-27：API／UI 回歸測試）
...
- [x] P7-2. 翻譯工具：英文／繁中／簡中、來源與目標雙箭頭切換、一般／技術／商務／中英對照；保留程式碼與表格；本機術語表。（2026-07-20）
- [x] P7-3. 模型選擇：一般聊天頂端直接列出預設模型與 fallback models，使用者可切換實際模型並保存；切換不會遺失其他選項。（2026-07-20）
