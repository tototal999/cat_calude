...
> 最後更新：2026-07-27；目前交付版本以本節與 release manifest 為準，後文較小體積數字為歷史紀錄。
...
- v7.0.4 onedir：85.6 MiB、2,695 個檔案；EXE SHA-256 已與 manifest 核對。
- 103/103 業務測試與 8/8 發布驗證通過，包含 GUI、公司部署、四格式文件及文件 Workflow。
...
- PPTX 文件助手新增「快速摘要（抽樣）」與「完整分析（全部投影片）」；完整模式每 6 張分批，
  所有批次成功後再彙整，coverage 必須完整才宣稱完成。無文字投影片仍計入總數並提示
  圖片／圖表未分析；批次失敗回報部分 coverage。實際內網語意品質仍待 `USER_TEST.md` 人工確認。
