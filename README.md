...
- **快速摘要（抽樣）**：長文件最多均勻抽取 12 個區塊，畫面會明確顯示抽樣 coverage，
  適合先快速掌握方向。
- **完整分析（全部投影片）**：僅支援 PPTX；每 6 張投影片交由目前選定的公司模型分析，
  再將所有批次摘要彙整為完整結果。只有全部批次成功才顯示完整 coverage，例如 `22/22`。
- 任一批次或最終彙整失敗時會回報已完成範圍，不會把部分結果宣稱為完整分析。
- 每張投影片都會被計數；沒有可擷取文字的圖片／圖表投影片會明確標示限制，
  目前不含 OCR 或視覺內容理解。
