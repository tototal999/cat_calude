...
產品定位調整為「桌面寵物 + 本機文件助手」，不是雲端 ChatGPT 的複製版。使用者不需要另開 Ollama、命令列或模型視窗；桌面寵物負責在背景管理已隨公司安裝包提供的本機推論服務。
...
- [ ] 離線安裝包完成後，不需 Ollama、命令列或網路，即可啟動一般聊天與文件助手。
- [ ] PDF、DOCX、PPTX、XLSX 各至少一份測試檔可產生含來源定位的回答；回答中每項事實均有對應區塊引用。
