...
cover(p.addSlide(), {
  ...
  subtitle: ["聊天", on("documents") && "文件問答",
             on("documents.meeting_pack") && "會議包",
             on("translate") && "翻譯", "安全結束"]
            .filter(Boolean).join(" ・ ") + "，三分鐘上手",
  date: "2026-07-21",
});
...
frame(s, "Agenda");
steps(s, [
  "ClaudeCat 能做什麼與切換公司核准的模型",
  on("quick_question") ? "快速提問與長回答" : "長回答",
  "切換到 LLM 交談介面",
  on("documents") && ("分析文件、文件問答" + (on("documents.meeting_pack") ? "與文件會議包" : "與來源引用")),
  "工具箱與 Skin：" + [on("json") && "JSON", on("translate") && "翻譯", "切換 Skin",
   (on("usage.claude") || on("usage.codex")) && "用量顯示"].filter(Boolean).join("、"),
  "桌寵管理" + (on("schedule") ? "、排程" : "") + "與安全結束",
...
{
  const tools = [on("json") && "JSON", on("translate") && "翻譯"].filter(Boolean);
  steps(s, ["右鍵點選桌面貓咪。", "選「交談（LLM 介面）…」。",
    tools.length ? `多輪對話，或切換 ${tools.join("／")}工具。` : "進行多輪對話。",
  ], { y: y + 0.1, gap: 0.72, size: 16 });
  const row1 = ["側欄對話紀錄：回看或繼續之前的對話",
                on("chat.attachments") && "附件：加入文字或 Excel 檔案"].filter(Boolean);
  const row2 = ["Markdown／程式碼複製",
                on("chat.export_pptx") && "簡報匯出 PPTX",
                "輸入 / 叫出快速提示詞"].filter(Boolean);
...
s = p.addSlide();
frame(s, "工具箱：" + [on("json") && "JSON", on("translate") && "翻譯"].filter(Boolean).join(" 與 "));
{
  const tops = [
    on("json") && { title: "JSON 工具（不需 LLM）",
      lines: ["Format／Minify／Validate", "搜尋與 JSONPath", "格式錯誤會顯示行與欄"] },
    on("translate") && { title: "翻譯工具（語意處理）",
      lines: ["來源可自動偵測或指定", "英／繁中／簡中，可用 ⇄ 互換", "一般／技術／商務／中英對照"] },
  ].filter(Boolean);
  // 只剩一張卡時改用整列寬度，避免右半邊留白
  tops.forEach((it, i) => card(s, {
    x: M + 0.35 + (tops.length > 1 ? i * 5.9 : 0), y: 1.45,
    w: tops.length > 1 ? 5.6 : 11.5, h: 2.5, bodySize: 15, ...it,
  }));
  if (on("translate")) {
  ...
  if (on("json")) note(s, "JSON 工具在 LLM 離線時仍可完全使用。", 6.3);
}
...
/* ========= 15. 桌寵管理與排程 ========= */
s = p.addSlide();
frame(s, on("schedule") ? "桌寵管理與排程" : "桌寵管理");
...
card(s, { x: M + 0.35, y: y + 0.2, w: 11.6, h: 3.3, title: "可以做的事", lines: [
  ...
  on("schedule") && "右鍵選「排程…」，可新增每日、每週或每小時提醒",
].filter(Boolean), bodySize: 16 });
...
p.writeFile({ fileName: "桌寵與LLM.pptx" }).then((f) => console.log("WROTE", f));
